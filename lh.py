...
def makePivotingStep(t, p1SCount, ebVar):
    ...
    for i in getRowNums(ebVar):
        if t.getItem(i, varToCol(ebVar)) < 0:
            ratio = -rational.Rational(t.getItem(i, 2)).__div__(t.getItem(i, varToCol(ebVar)))
    ...
    for j in range(2, t.getNumCols() + 1):
        newVal = rational.Rational(t.getItem(lbVarRow, j)).__div__(abs(lbVarCoeff))
...
def normalizeEquilibrium(eq):
    ...
    def normalizeEqPart(eqPart):
        probSum = functools.reduce(lambda x, y: x + y, eqPart, 0)
