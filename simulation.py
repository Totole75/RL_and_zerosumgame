...
players = [regret_matching(loss_array, step_number), perturbed_fictitious_play(-loss_array.T, step_number)]
...
mean_reward = np.cumsum(rewards_array) / np.arange(1, step_number+1)
plt.plot(range(erased_values, step_number), mean_reward[erased_values:])

plt.legend(["Optimal value", "Mean loss for player 1"])
plt.xlabel("Rounds")
